    ...
    : ICommandHandler<CreateEnrollmentCommand, EnrollmentId>
{
    ...
    {
        CreateEnrollmentDto enrollmentDto = command.Dto;
        var userId = new ExternalUserId(enrollmentDto.UserId);
        var courseId = new ExternalCourseId(enrollmentDto.CourseId);
        ...
        if (!userExists)
        {
            _logger.LogWarning("User {UserId} does not exist", enrollmentDto.UserId);
        ...
        if (!courseExists)
        {
            _logger.LogWarning("Course {CourseId} does not exist", enrollmentDto.CourseId);
            return Result.Failure<EnrollmentId>(new Error(
                "Course.NotFound",
        ...
        var enrollment = Enrollment.Create(userId, courseId, enrollmentDto.ExpiresAt);

        _dbContext.Enrollments.Add(enrollment);
        await _dbContext.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Enrollment {EnrollmentId} created for UserId: {UserId}, CourseId: {CourseId}",
            enrollment.Id.Value, enrollmentDto.UserId, enrollmentDto.CourseId);
