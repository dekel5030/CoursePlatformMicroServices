...
public class CourseConfiguration : IEntityTypeConfiguration<Course>
{
    public void Configure(EntityTypeBuilder<Course> builder)
    {
        ...
        builder.OwnsMany(course => course.Enrollments, enrollmentBuilder =>
        {
            enrollmentBuilder.ToTable("enrollments");
            enrollmentBuilder.HasKey(enrollment => new { enrollment.CourseId, enrollment.StudentId });
            enrollmentBuilder.Property(enrollment => enrollment.StudentId)
                .HasConversion(
                    id => id.Value,
                    value => new StudentId(value));
        });

        builder.Navigation(course => course.Enrollments)
            .UsePropertyAccessMode(PropertyAccessMode.Field);
