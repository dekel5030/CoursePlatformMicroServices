...
export default function HomePage() {
  ...
  const categories = useMemo(() => {
    ...
    courses.forEach((c) => c.title && set.add(c.title)); // Assuming title serves as category for now, or use a real category field if available
    ...
    <div className="flex flex-col">
      ...
      <header className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6 border-b border-border pb-8">
        <div className="space-y-2 max-w-2xl">
          <h1 className="text-4xl font-bold tracking-tight text-foreground">{t('navbar.catalog')}</h1>
          <p className="text-lg text-muted-foreground">
            Explore our curated collection of architectural and engineering courses. 
            From BIM mastery to computational design.
          </p>
        </div>
      ...
      <div className="space-y-8">
        {categories.length > 0 && (
            <div className="flex overflow-x-auto pb-4 gap-2 scrollbar-none" aria-label="Categories">
            <Button
        ...
        <main>
            ...
            >
                {filtered.map((course) => (
                <motion.div key={course.id.value} variants={item}>
