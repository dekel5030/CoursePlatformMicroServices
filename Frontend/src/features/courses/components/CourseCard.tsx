...
import { DifficultyLevel, CourseStatus } from "../types/CourseModel";
import { Card, CardContent, CardHeader, Badge, Avatar } from "@/components";
...
export default function CourseCard({ course }: Props) {
  ...
  const getDifficultyLabel = (difficulty: DifficultyLevel) => {
    const enumName = DifficultyLevel[difficulty];
    const key = typeof enumName === "string" ? enumName.toLowerCase() : "";
    // If key is empty or invalid, just return the enum name
    if (!key) return enumName || "";
    return t(`courses:difficulty.${key}`, { defaultValue: enumName || "" });
  };
